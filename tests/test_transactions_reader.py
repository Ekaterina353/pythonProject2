...
def test_read_transaction_csv_success():
    # Мокируем pd.read_csv чтобы вернуть DataFrame
    mock_df = pd.DataFrame([{'date': '2023-11-15', 'amount': 100, 'description': 'Test'}])
...
def test_read_transaction_excel_success():
    ...
    with patch('transactions_reader.pd.read_excel', return_value=mock_df):
